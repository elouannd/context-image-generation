...
| TokenReply `grok-imagine-image` | Experimental | A minimal, text-only OpenAI Images request targets `https://api.tokenreply.com/v1/images/generations`. | No live test-key generation has been recorded. The adapter omits image size and references until TokenReply's accepted size/resolution field and response shape are live-verified. |
...
4. TokenReply `grok-imagine-image`, including whether `size`, `resolution`, or neither is accepted and whether the response contains base64, a URL, or another supported shape.
